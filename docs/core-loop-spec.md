...
> **Assert the teaching claim, not that it rendered — and where a layer can't run in Go, contract-test the seam.** Building twenty-one example notebooks pressure-tested this at scale, and the finding split two ways. First: nearly every notebook shipped a real bug that `go test` passed straight over, because the *first* test asserted a mechanism ("it renders an SVG", "the cell ran") rather than the consequence the notebook exists to show. The spectrogram was the sharpest — its readout metric was *inverted* (it rewarded a rectangular window, the opposite of the leakage lesson its own prose taught); the code ran, the tests were green, and the number said the opposite of the text. The rule that catches this is §8's first line, applied earlier: write the test that asserts the *claim* (Hann reduces leakage; energy drift is Euler≫Verlet; the CLT bell un-converges) **before** you trust the demo, not after — and drive the real output (a headless sweep, a metric probe, a screenshot) because the claim is usually a number or a picture, not a type. Second: some bugs are structurally invisible to Go — the `[object Object]` control and the inert `//notebook:row=` directive both lived in the client JS, which has no DOM under `go test` and shipped uncovered for the project's whole life. The fix is not a browser rig (it would forfeit the stdlib discipline and add a flaky CI surface); it is a **contract test** that reads *both sides as text* and asserts they agree — every widget `Kind` the analyzer emits must have a `buildControl` branch, the `row=` directive must be read and grouped. It executes no JS, adds no dependency, and fails the moment the two sides drift. When a seam crosses a language boundary the test harness can't cross, test the *contract at the seam*, not the far side's behavior.

> **A kill criterion must be falsifiable, not merely achievable — state the anti-pass.** This is the same rule applied *before* the code, at framing time, and it is the correctly-generalized KC4 lesson. The KC4 mistake was not only inventing a number; it was writing a criterion that could be *satisfied without the thing being true*. A criterion that is merely achievable gets achieved — a demo is staged that ticks the box while the claim it stands for stays false: the host page "drives the notebook" but only because our own UI is still mounted and restyled; the dataset "crosses the wire" but a rebuild slipped in. So every KC states its **anti-pass** alongside its pass — the specific way a demo could masquerade as the result — and the KC has not passed until you have ruled the anti-pass out by observation. Behavioral over numeric where the question is structural: a millisecond target on "does a stranger's page hold this notebook" would be the KC4 error committed knowingly, a number invented to look like evidence for a structural claim it cannot bear. When a framing doc asserts a claim, label what in it is *evidence* (a seam that exists in the tree) and what is *claim* (unexercised prose), and name the specific fact that keeps the claim unproven — so no one reads the prose in six months and takes it for a fact.
