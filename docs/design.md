...
The renderer emits *declarative grips*; the runtime binds them to leaf writes. And the grip names its leaf without a string — `Draggable[T]` carries an opaque runtime-stamped token, so `ctrl.Grip(i)` is typechecked. **The leaf's identity rides along with the value.** That is only possible because widgets are values.
