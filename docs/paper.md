...
The system is deliberately small. The design records six corrections that the work forced — including the withdrawal of its own once-"strongest claim" (compile-time-checked SQL) — and a recurring finding, *"running is not passing,"* that turns out to be the paper's real contribution: a system producing a thing is not the same as the thing reaching anyone, and the discipline of verifying by observed consequence caught bugs that green test suites passed over. A corpus of ~39 notebooks (34 running live in the browser) exercises the design across queueing theory, statistics, physics, distributed systems, and HPC, and each was built as a falsification attempt: 19 of them across GPU/DFT/ODE/gradient-descent/cellular-automata subjects required **zero engine changes and zero violations of "a cell is a function."**
...
## 8. Data that doesn't fit, and the claim that was withdrawn
...
The design's once-**"strongest claim" was compile-time-checked SQL** — a SQL string typechecked against a struct schema at build time. It is **withdrawn, not deferred.** The `taxi` notebook demonstrates that typed Go operations over `Rel[T]` deliver the same guarantee (rename a column and the cells fail to compile) using the compiler that already exists — no SQL parser (the single largest piece of engineering in the original design), no cgo, no dialect. A notebook that genuinely wants SQL calls DuckDB from an ordinary helper and pays the cgo cost *locally*, rather than imposing it on the toolchain to check a string the design makes unnecessary. Withdrawing it closed the one scar in the design: **no cgo, static binary intact, all four topologies clean.**
...
The ~39-notebook corpus (34 live in the browser) is not a demo gallery; it is how the design is pressure-tested. Each notebook was built to put one mechanism on stage or to reproduce a real result, and the building is where the design is falsified. Two structural findings:
...
**Why it holds together.** The same cut kept working at every layer, and every "how do we avoid X" turned out to be "you already avoided it three decisions ago." That is the opposite of the accretion pattern that grows frameworks. Fewer layers is not a gap to be filled; it is the point. The system is one new concept (`Prev` + `Tick`), six corrections the work forced (per-widget reconciliation; cacheability derived not declared; the edit log real but only for stateful notebooks; purity and portability are different verdicts; the grip token justified structurally; and SQL withdrawn), and a testing discipline earned the hard way.
