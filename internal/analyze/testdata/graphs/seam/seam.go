...
// Order in which seams are removed, and how long finding it took. A cell with
// two named results: `order` feeds the carve and the heat map; `elapsed` feeds
// the cost readout. Both are edges — one cell, two outputs, both consumed.
//
// Depends on the image, not the slider — the entire reason this notebook needs
// no cache.
func seamOrder(img Frame) (order SeamMap, elapsed time.Duration) {
	start := time.Now()
...
// Time spent finding seams, as a readable card.
// The original print()s this — but parallel cells share one stdout, so a notebook
// that fans out across goroutines cannot own a per-cell console. You return a value
// instead, and here that value is the second output of seamOrder flowing in as its
// own edge. This cell is why "return a value, don't print" is a rule, not a nag.
func precomputeCost(elapsed time.Duration) (cost Markdown) {
	return Markdown("**Seam precompute:** " + elapsed.Round(time.Millisecond).String())
}
